...
Focus on creating a prompt that would result in a high-quality 3D model that closely matches the input image.`;
  ...
  private createUserPrompt(): string {
    ...
    if (this.includeTechnicalSpecs) {
      prompt += "Include technical specifications like polygon count, topology, and geometric details. ";
    }
    
    if (this.includeMaterials) {
      prompt += "Describe materials, textures, surface properties, and color information. ";
    }
  ...
  private textureToBase64(texture: Texture): string | null {
    try {
      // Convert texture to base64
      // This is a simplified conversion - in practice, you might need more sophisticated conversion
      const canvas = texture.getImageData();
      if (!canvas) {
        log.e("Failed to get image data from texture");
        return null;
      }
      
      // For now, we'll use a placeholder approach
      // In a real implementation, you'd convert the texture data to base64
      log.d("Converting texture to base64...");
      
      // This is a simplified approach - you may need to implement proper texture-to-base64 conversion
      // based on your specific texture format and Lens Studio version
      return "data:image/jpeg;base64,";
      
    } catch (error) {
      log.e("Error converting texture to base64: " + error);
      return null;
    }
  }
  ...
  public updateSettings(style: string, detail: number, technical: boolean, materials: boolean, lighting: boolean) {
